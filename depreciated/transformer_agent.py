...
class Transformer:
    def __init__(self,
        ...
        self.f_safety = 1.2  # long lifetime of transformer means high safety
        self.p_over = 10
        self.capacity = None
    ...
    def set_transformer_power_capacity(self):
        ...
        """
        # TODO remove diversity factor it is already included in h0 profiles, include power factor of 0.9 for kW -> kVA
        # TODO and take the max capacity of the h0 profile + 100
        self.capacity = self.get_c_diversity() * sum(self.customers_contracted_power) * self.f_safety + self.p_over
        # self.capacity = 0.5 * 10 * self.f_safety + self.p_over

        # print("Transformer with a capacity of {} kW".format(self.capacity))

        # self.kva = P_peak / 0.9 (power factor) * F Safety (1.2) + Pover
